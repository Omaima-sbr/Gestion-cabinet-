...
public class PatientService {
    ...
    // ✅ Méthode helper pour récupérer l'utilisateur (peut retourner null en mode dev)
    ...
    public PatientDTO createPatient(PatientDTO dto) {
        ...
        // ✅ Récupérer le cabinet si utilisateur connecté
        Utilisateur currentUser = getCurrentUser();
        if (currentUser != null && currentUser.getCabinet() != null) {
            patient.setCabinet(currentUser.getCabinet());
        } else {
            ...
            patient.setCabinet(defaultCabinet);
        }
    ...
    // ✅ VERSION CORRIGÉE - Sans authentification requise
    public Page<PatientDTO> getAllPatients(Pageable pageable) {
        Utilisateur currentUser = getCurrentUser();

        if (currentUser != null && currentUser.getCabinet() != null) {
            // Mode authentifié : filtrer par cabinet
            return patientRepository.findByCabinet(currentUser.getCabinet(), pageable)
                    .map(this::mapToDTO);
        } else {
            // Mode dev : retourner tous les patients
    ...
    public List<PatientDTO> searchByNomPrenom(String nom, String prenom) {
        ...
        if (currentUser != null && currentUser.getCabinet() != null) {
            ...
            if (prenom != null && !prenom.isEmpty()) {
                ...
                patients = patientRepository.findByNomContainingIgnoreCaseAndCabinet(nom, currentUser.getCabinet());
            }
        } else {
            // Mode dev : recherche globale
            if (prenom != null && !prenom.isEmpty()) {
                patients = patientRepository.findByNomContainingIgnoreCaseAndPrenomContainingIgnoreCase(nom, prenom);
            } else {
                patients = patientRepository.findByNomContainingIgnoreCase(nom);
            }
        }
    ...
    public List<PatientDTO> advancedSearch(PatientSearchDTO searchDTO) {
        // Implémentation de la recherche avancée avec Specification
        return List.of(); // À implémenter selon vos besoins
    }
