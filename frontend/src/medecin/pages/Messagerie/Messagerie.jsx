// src/medecin/pages/messagerie/Messagerie.jsx
import React, { useState, useEffect, useRef } from 'react';
import medecinMessagerieService from '../../services/medecinMessagerieService';
...
const Messagerie = () => {
    ...
    const loadMessages = async () => {
        ...
        try {
            const data = activeTab === 'recus'
                ? await medecinMessagerieService.getMessagesRecus()
                : await medecinMessagerieService.getMessagesEnvoyes();

            setMessages(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error('❌ Erreur chargement messages:', err); // ✅ FIX: Utiliser err
    ...
    const loadSecretaires = async () => {
        try {
            const data = await medecinMessagerieService.getSecretairesDisponibles();
            setSecretaires(Array.isArray(data) ? data : []);
        } catch (err) {
            console.error('⚠️ Erreur chargement secrétaires:', err); // ✅ FIX: Utiliser err
            setSecretaires([]);
        }
    };
    ...
    const loadConversation = async (utilisateurId) => {
        try {
            const data = await medecinMessagerieService.getConversation(utilisateurId);
    ...
    const handleMarquerLu = async (id) => {
        try {
            await medecinMessagerieService.marquerCommeLu(id);
            loadMessages();
        } catch (err) {
    ...
    const handleSupprimerMessage = async (messageId) => {
        ...
        try {
            await medecinMessagerieService.supprimerMessage(messageId);
    ...
    const handleNewConversation = (secretaire) => {
        setSelectedDestinataire({
            id: secretaire.id,
            nom: `${secretaire.nom || ''} ${secretaire.prenom || ''}`.trim()
        });
        setConversation([]);
        setSelectedMessage(null);
    };
    ...
    const handleEnvoyerMessage = async (e) => {
        ...
        try {
            const newMsg = await medecinMessagerieService.envoyerMessage({
    ...
    const handleAudioRecorded = async (audioBlob) => {
        ...
        try {
            ...
            const newMsg = await medecinMessagerieService.envoyerMessageAvecFichier({
                idDestinataire: selectedDestinataire.id,
                objet: 'Message vocal',
                    ...
                    {activeTab === 'nouveau' ? (
                        <div className="messenger-contacts">
                            <div className="messenger-contacts-header">
                                <h3>📋 Sélectionnez une secrétaire</h3>
                            </div>
                            {secretaires.length === 0 ? (
                                <div className="messenger-empty">Aucune secrétaire disponible</div>
                            ) : (
                                secretaires.map(secretaire => (
                                    <div
                                        key={secretaire.id}
                                        className={`messenger-contact ${selectedDestinataire?.id === secretaire.id ? 'selected' : ''}`}
                                        onClick={() => handleNewConversation(secretaire)}
                                    >
                                        <Avatar name={`${secretaire.nom} ${secretaire.prenom}`} />
...
// ✅ FIX: Export par défaut pour éviter l'erreur Fast Refresh
