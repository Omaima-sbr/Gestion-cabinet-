...
export default function Sidebar() {
  ...
  // CORRECTION ICI : Ajout du préfixe "/admin" devant tous les chemins
  const menuItems = [
  ...
  // CORRECTION ICI : Pour que le bouton reste allumé même si on est dans une sous-page
  const getActivePath = (path) => {
    if (path === "/admin" && location.pathname === "/admin") return true; // Cas strict pour le dashboard
    if (path !== "/admin" && location.pathname.startsWith(path)) return true; // Cas inclusif pour les autres
    return false;
  };
