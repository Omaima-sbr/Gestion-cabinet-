...
export default function MedicalManagement() {
  ...
  const loadCabinets = async () => {
    try {
      ...
      setCabinets([]); // En cas d'erreur, on remet un tableau vide pour éviter le crash
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSearch = async (value) => {
    ...
    if (value.trim() === "") {
      ...
      try {
        ...
        // ✅ CORRECTION 2 : Sécurité sur la recherche aussi
        setCabinets(Array.isArray(results) ? results : []);
      } catch (error) {
  ...
  return (
      <div className="flex flex-col overflow-y-auto p-4 md:p-8 lg:p-12 font-sans bg-gray-50 min-h-screen">
          ...
          {isLoading ? (
              ...
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  {/* CORPS DU TABLEAU */}
                  <tbody>
                  {/* ✅ CORRECTION 3 : Vérification supplémentaire avant le map */}
                  {Array.isArray(cabinets) && cabinets.map((c, index) => (
                      <tr
                          key={c.id}
        ...
        {/* ✅ CORRECTION 4 : Style tag corrigé */}
        <style>{`
        @keyframes fadeIn {
